...
> **🏆 竞赛项目说明**  
> 本作品为参赛项目，选题来源于团队成员所在高校的实际需求。  
> 尽管在技术路线上采用了合理的低成本方案（本地小模型 + 外部 LLM 降级链路），
> 但就**商业价值**而言，作者认为其更偏向**校园场景的解决方案**，
> 与现有成熟的 OA/审批 SaaS 相比，在通用性和规模化上不具备明显商业优势，
> 更适合作为**开源参考实现**或**特定院校的定制化工具**。
...
- 🖼️ 新增 AuthImage 组件，修复所有详情页认证图片 403
- 📝 审批面板 UI 统一（查看详情 → 弹窗审批），去除表格内直接审批按钮
- 🧠 合规分析面板始终可见，AI 填写意见按钮去重
- 🌐 字段中文汉化补全（getFieldLabel 统一映射）
- 🔤 按钮 emoji 精简、文案一致性优化
