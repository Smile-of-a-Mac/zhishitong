...
// ===== 供应商预设 =====
const PRESETS: Record<string, { label: string; api_base: string; ocr_models: string[]; fill_models: string[] }> = {
  aliyun: {
    label: '阿里百炼',
...
export default function AdminApiKeysPage() {
  ...
  const [showAdd, setShowAdd] = useState<'ocr' | 'fill' | null>(null)
  ...
  const fetch = async () => {
    const [ocr, fill] = await Promise.all([
    ...
    setOcrKeys(ocr); setFillKeys(fill)
  }
  useEffect(() => { fetch() }, [])
  ...
  const modelOptions = provider
    ? (showAdd === 'ocr' ? PRESETS[provider]?.ocr_models : PRESETS[provider]?.fill_models) || []
    : []
    ...
    <GlassCard size="sm" style={{ marginTop: 12, background: 'var(--glass-bg)' }}>
      ...
      <div style={{ marginBottom: 8 }}>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>
          ④ 选择模型 {keyType === 'ocr' ? '(多模态 OCR)' : '(JSON 填充)'}
        </label>
